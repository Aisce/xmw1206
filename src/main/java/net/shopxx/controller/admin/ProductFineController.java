 ...
 * @version 6.1
 */
@Controller("adminProductFineController")
@RequestMapping("/admin/product_fine")
public class ProductFineController extends BaseController {
	...
	public String list(Product.Type type, Long productCategoryId, Long brandId, Long productTagId, Boolean isActive, Boolean isMarketable, Boolean isList, Boolean isTop, Boolean isOutOfStock, Boolean isStockAlert, Pageable pageable, ModelMap model) {
		...
		model.addAttribute("page", productService.findPage(type, null, null, productCategory, null, brand, null, productTag, null, null, null, null, null, isMarketable, isList, isTop, true, isActive, isOutOfStock, isStockAlert, null, null, pageable, null));
		return "admin/product_fine/list";
	}
	 ...
	 * 设置优秀商品
	 */
	@PostMapping("/set_fine")
	public ResponseEntity<?> setFine(Long[] ids) {
		productService.setFine(ids);
		return Results.OK;
	}
	 ...
	 * 取消优秀商品
	 */
	@PostMapping("/cancel_fine")
	public ResponseEntity<?> cancelFine(Long[] ids) {
		productService.cancelFine(ids);
		return Results.OK;
	}
}