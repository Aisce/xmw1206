 ...
 * FileId: AO5RY3vKIgxnV9nqKaWWd6O7+rZEYEcd
 */
...
public class OrderController extends BaseController {
	...
	public ResponseEntity<?> addProgress(Long productId, OrderProgress orderProgressForm, @ModelAttribute(binding = false) Order order, @CurrentUser Business currentUser) {
		...
		if (order == null || order.getStatus() != Order.Status.PENDING_SHIPMENT) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		if (!isValid(orderProgressForm)) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		orderProgressForm.setProduct(product);
		orderService.addProgress(order, orderProgressForm);
	...
	public @ResponseBody List<Map<String, Object>> listProgress(Long productId, Long orderId, @CurrentUser Business currentUser) {
		...
		List<OrderProgress> orderProgress = orderProgressService.list(orderId, productId);
		for (OrderProgress orderProgres : orderProgress) {
			Map<String, Object> item = new HashMap<>();
			item.put("content", orderProgres.getContent());
			item.put("remark", orderProgres.getRemark());
			item.put("createDate", orderProgres.getCreatedDate().getTime());
			data.add(item);
		}
		return data;
	}
