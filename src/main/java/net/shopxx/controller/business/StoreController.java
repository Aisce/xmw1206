 ...
 * FileId: 9acqL+n7oAoQeWZePc7n3/UwXgQFeimF
 */
...
public class StoreController extends BaseController {
	 ...
	 * 生成编号
	 */
	@PostMapping(path = "/create_code", produces = MediaType.APPLICATION_JSON_VALUE)
	@JsonView(BaseEntity.BaseView.class)
	public ResponseEntity<?> createCode(Long storeCategoryId) {
		
		StoreCategory storeCategory = storeCategoryService.find(storeCategoryId);
		//获取编号(前两位)
		Integer category_code = storeCategory.getCategory_code();
		//获取同类厂家数量
		Long count = storeService.count(storeCategoryId);
		
		String str1 = "";
		
		if(category_code < 10) {
			str1 = "0"+Integer.toString(category_code);
		}else {
			str1 = Integer.toString(category_code);
		}
		...
		String number = str1 + str + danorshuang;
	...
	public String register(@CurrentStore Store currentStore, ModelMap model) {
		...
		String number = "";
		if (storeCount < 10) {
			number = "0000" + storeCount;
		} else if (storeCount < 100) {
			number = "000" + storeCount;
		} else if (storeCount < 1000) {
			number = "00" + storeCount;
		} else if (storeCount < 10000) {
			number = "0" + storeCount;
		} else {
			number = "" + storeCount;
		}
	...
	public ResponseEntity<?> register(String number,String name, String email, String mobile, Long storeRankId, Long storeCategoryId, Long[] productCategoryIds, @CurrentUser Business currentUser) {
	...
	public ResponseEntity<?> setting(Store store, @CurrentStore Store currentStore) {
		...
		currentStore.setSclxr(store.getSclxr());
		currentStore.setSclxfs(store.getSclxfs());
