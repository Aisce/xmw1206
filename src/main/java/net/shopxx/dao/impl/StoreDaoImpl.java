...
public class StoreDaoImpl extends BaseDaoImpl<Store, Long> implements StoreDao {
	...
	public Long count(Long storeCategoryId) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Store> criteriaQuery = criteriaBuilder.createQuery(Store.class);
		Root<Store> root = criteriaQuery.from(Store.class);
		criteriaQuery.select(root);
		...
		if (storeCategoryId != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("storeCategory"), storeCategoryId));
		}
		criteriaQuery.where(restrictions);
	...
	@Override
	public Page<Store> findPage(Store.Type type, Store.Status status, Boolean isFine, Boolean isEnabled, Boolean hasExpired, Pageable pageable) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Store> criteriaQuery = criteriaBuilder.createQuery(Store.class);
		Root<Store> root = criteriaQuery.from(Store.class);
		criteriaQuery.select(root);
	...
	@Override
	public Page<Store> findPage(Store.Type type, Store.Status status, Boolean isFine, Boolean isEnabled, Boolean hasExpired, Pageable pageable,Long storeCategoryId,String name) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Store> criteriaQuery = criteriaBuilder.createQuery(Store.class);
		Root<Store> root = criteriaQuery.from(Store.class);
		criteriaQuery.select(root);
	...
	public void setFine(Long[] ids) {
		...
		String jpql = "update Store store set store.isFine = true where store.isFine = false and store.id in (:ids)";
	...
	public void cancelFine(Long[] ids) {
		...
		String jpql = "update Store store set store.isFine = false where store.isFine = true and store.id in (:ids)";
		entityManager.createQuery(jpql).setParameter("ids", Arrays.asList(ids)).executeUpdate();
	}
