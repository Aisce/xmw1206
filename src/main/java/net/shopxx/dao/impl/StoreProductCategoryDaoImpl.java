 ...
 * FileId: /16pvjJe5hxJ5ps+RE/0qJCUwGF1eO+m
 */
...
public class StoreProductCategoryDaoImpl extends BaseDaoImpl<StoreProductCategory, Long> implements StoreProductCategoryDao {
	...
	//使用名称获取数据
	@Override
	public List<StoreProductCategory> findname(StoreProductCategory storeProductCategory, Store store, boolean recursive, Integer count) {
		TypedQuery<StoreProductCategory> query;
		if (recursive) {
			if (storeProductCategory != null) {
				String jpql = "select storeProductCategory from StoreProductCategory storeProductCategory where storeProductCategory.store = :store and storeProductCategory.name  = :name order by storeProductCategory.grade asc";
				query = entityManager.createQuery(jpql, StoreProductCategory.class).setParameter("name", storeProductCategory.getName()).setParameter("store", storeProductCategory.getStore());
			} else {
				String jpql = "select storeProductCategory from StoreProductCategory storeProductCategory where storeProductCategory.store = :store order by storeProductCategory.grade asc";
				query = entityManager.createQuery(jpql, StoreProductCategory.class).setParameter("store", store);
			}
	...
	public void saveStoreProductCategory(StoreProductCategory storeProductCategory) {
		super.persist(storeProductCategory);
	}
