 ...
 */
@Service
public class NewLaunchServiceImpl extends BaseServiceImpl<NewLaunch, Long> implements NewLaunchService {
	...
	public Page<NewLaunch> search(String keyword, Pageable pageable) {
	...
	public long viewHits(Long id) {
		...
		Ehcache cache = cacheManager.getEhcache(NewLaunch.HITS_CACHE_NAME);
		...
		try {
			...
			if (element != null) {
				...
				NewLaunch find = newLaunchDao.find(id);
				if (find == null) {
					return 0L;
				}
				hits = find.getHits() + 1;
	...
	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "newLaunch", condition = "#useCache")
	public List<NewLaunch> findList(Boolean isPublication, Integer count, List<Filter> filters, List<Order> orders, boolean useCache) {
		return newLaunchDao.findList(isPublication, count, filters, orders);
	}
	@Override
	public Page<NewLaunch> findPage(NewLaunch entity, Pageable pageable) {
		// TODO Auto-generated method stub
		return newLaunchDao.findPage(entity, pageable);
	}
}