 ...
 * FileId: jD/BzIRLYB/DOudgO4HsrSD+qihN+vBI
 */
...
public class ProductCategoryServiceImpl extends BaseServiceImpl<ProductCategory, Long> implements ProductCategoryService {
	...
	@Transactional
	@CacheEvict(value = { "product", "productCategory" }, allEntries = true)
	public List<ProductCategory> findInIds(String[] ids){
		...
		for(int i=0;i<strArrNum.length;i++) {
			Long id= strArrNum[i];
			ProductCategory find = super.find(id);
	...
	public List<ProductCategory> findname(ProductCategory storeProductCategory, Store store, boolean recursive, Integer count){
		return productCategoryDao.findname(storeProductCategory, store, recursive, count);
