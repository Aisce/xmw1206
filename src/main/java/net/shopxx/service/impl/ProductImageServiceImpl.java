 ...
 * FileId: GJMd6PmeY7SIo+XLMn2xkkqEi+CKAuS5
 */
...
public class ProductImageServiceImpl implements ProductImageService {
	 ...
	 *            原文件类型
	 */
	private void addTask(final StoragePlugin storagePlugin, final String sourcePath, final File tempFile, final String contentType) {
		taskExecutor.execute(new Runnable() {
			public void run() {
				try {
					storagePlugin.upload(sourcePath, tempFile, contentType);
	...
	@Override
	public ProductFile generateFile(MultipartFile multipartFile) {
		Assert.notNull(multipartFile, "[Assertion failed] - multipartFile is required; it must not be null");
		Assert.state(!multipartFile.isEmpty(), "[Assertion failed] - multipartFile must not be empty");
		...
		try {
			...
			for (StoragePlugin storagePlugin : pluginService.getStoragePlugins(true)) {
				File tempFile = new File(FileUtils.getTempDirectory(), uuid + "." + TEMP_FILE_EXTENSION);
				multipartFile.transferTo(tempFile);
				addTask(storagePlugin, sourcePath, tempFile, multipartFile.getContentType());

				ProductFile productFile = new ProductFile();
				productFile.setSource(storagePlugin.getUrl(sourcePath));
				return productFile;
			}
		} catch (IllegalStateException e) {
			throw new RuntimeException(e.getMessage(), e);
