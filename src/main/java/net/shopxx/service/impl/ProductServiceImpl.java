 ...
 * FileId: 62t+PUu3bEbxm/HgxRuk5o+14U8AucFO
 */
...
public class ProductServiceImpl extends BaseServiceImpl<Product, Long> implements ProductService {
	...
	public List<Product> findList(String numbers, Product.OrderType orderType, Integer count, List<Filter> filters, List<Order> orders){
		return productDao.findList(numbers, orderType, count, filters, orders);
	}
	...
	public List<Product> isfindList( Store store, ProductCategory productCategory,String name,String xinghao,String fengzhuang,String zhiliang) {
		return productDao.isfindList(store, productCategory, name, xinghao, fengzhuang, zhiliang);
		
	}
	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "product", condition = "#useCache")
	public List<Product> findList(Long productId,Product.Type type, Long storeId, Long productCategoryId, Long storeProductCategoryId, Long brandId, Long promotionId, Long productTagId, Long storeProductTagId, Map<Long, String> attributeValueMap, BigDecimal startPrice, BigDecimal endPrice, Boolean isMarketable,
			Boolean isList, Boolean isTop, Boolean isActive, Boolean isOutOfStock, Boolean isStockAlert, Boolean hasPromotion, Product.OrderType orderType, Integer count, List<Filter> filters, List<Order> orders, boolean useCache) {
		Store store = storeDao.find(storeId);
		if (storeId != null && store == null) {
			return Collections.emptyList();
		...
		return productDao.findList(product,type, store, productCategory, storeProductCategory, brand, promotion, productTag, storeProductTag, map, startPrice, endPrice, isMarketable, isList, isTop, isActive, isOutOfStock, isStockAlert, hasPromotion, orderType, count, filters, orders);
	}
	...
	public Page<Product> findPage(Product.Type type, Store.Type storeType, Store store, ProductCategory productCategory, StoreProductCategory storeProductCategory, Brand brand, Promotion promotion, ProductTag productTag, StoreProductTag storeProductTag, Map<Attribute, String> attributeValueMap,
		...
		return productDao.findPage(type, storeType, store, productCategory, storeProductCategory, brand, promotion, productTag, storeProductTag, attributeValueMap, promotionPluginId, startPrice, endPrice, isMarketable, isList, isTop, isFine, isActive, isOutOfStock, isStockAlert, hasPromotion, orderType,
				pageable, isHot);
	}
	...
	public Product modify(Product product, Sku sku) {
		...
		setValue(product);
		// copyProperties(product, pProduct, "sn", "type", "score", "totalScore", "scoreCount", "reviews", "consultations", "productFavorites", "skus", "store");
	...
	@Override
	public List<Product> findCopysn(Store currentStore,String copySn) {
		// TODO Auto-generated method stub
		return productDao.findByCopySn(currentStore, copySn);
	}

	@Override
	public Page<Product> findPage(Pageable pageable, Boolean isSample,Boolean isActive,Boolean isMarketable) {
		// TODO Auto-generated method stub
		return productDao.findPage(pageable, isSample,isActive,isMarketable);
	}

	@Override
	public Page<Product> findPageStoreId(Pageable pageable, MechanismStore jcStore, MechanismStore bzStore) {
		// TODO Auto-generated method stub
		return productDao.findPageStoreId(pageable, jcStore, bzStore);
	}
