...
<head>
	...
	<title>${message("admin.business.view")} - 北京芯梦国际科技有限公司</title>
							...
							<div class="row">
								<div class="col-xs-12 col-sm-6">
									<dl class="items dl-horizontal">
										...
										<dd>${mechanism.email}</dd>
										<dt>${message("Business.mobile")}:</dt>
										<dd>${mechanism.mobile}</dd>
										...
										<dd>${mechanism.createdDate?string("yyyy-MM-dd HH:mm:ss")}</dd>
										<dt>${message("User.lastLoginIp")}:</dt>
										<dd>${mechanism.lastLoginIp!"-"}</dd>
										<dt>${message("User.lastLoginDate")}:</dt>
										<dd>${(mechanism.lastLoginDate?string("yyyy-MM-dd HH:mm:ss"))!"-"}</dd>
									</dl>
								</div>
							</div>
						</div>
										...
										<dl class="items dl-horizontal">
											[#list businessAttributes as businessAttribute]
												...
												[#elseif businessAttribute.type == "SELECT"]
													<dd>${mechanism.getAttributeValue(businessAttribute)}</dd>
												[#elseif businessAttribute.type == "CHECKBOX"]
													<dd>
														[#list mechanism.getAttributeValue(businessAttribute) as option]
															${option}
														[/#list]
													</dd>
												[/#if]
