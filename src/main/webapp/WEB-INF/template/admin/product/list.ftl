...
<head>
	...
	<title>${message("admin.product.list")} - 北京芯梦国际科技有限公司</title>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$delete.on("success.shopxx.delete", function() {
					$shelves.add($shelf).add($setFine).add($setHot).attr("disabled", true);
				});
				...
				// 设置优秀产品
				$setFine.click(function() {
					bootbox.confirm("${message("admin.product.setFineConfirm")}", function(result) {
						if (result == null || !result) {
							return;
						}
						
						$.post("${base}/admin/product_fine/set_fine", $("input[name='ids']").serialize(), function() {
							location.reload(true);
						});
					});
				});
				...
				$setHot.click(function() {
					bootbox.confirm("${message("admin.product.setHotConfirm")}", function(result) {
						if (result == null || !result) {
							return;
						}
						
						$.post("${base}/admin/product_hot/set_hot", $("input[name='ids']").serialize(), function() {
							location.reload(true);
						});
					});
				});
				...
				// ID多选框
				$ids.change(function() {
					$shelves.add($shelf).add($setFine).add($setHot).attr("disabled", $("input[name='ids']:checked").length < 1);
				});
	...
	<main>
		<div class="container-fluid">
			...
			<form action="${base}/admin/product/list" method="get">
				...
				<input name="isFine" type="hidden" value="[#if isFine??]${isFine?string("true", "false")}[/#if]">
				<input name="isHot" type="hidden" value="[#if isHot??]${isHot?string("true", "false")}[/#if]">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									...
									<button id="setFine" class="btn btn-default" type="button" data-toggle="setFine" disabled>
										<i class="iconfont icon-moreunfold"></i>
										${message("admin.product.setFine")}
									</button>
									<button id="setHot" class="btn btn-default" type="button" data-toggle="setHot" disabled>
										<i class="iconfont icon-moreunfold"></i>
										${message("admin.product.setHot")}
									</button>
									<div class="btn-group">
										...
										<ul class="dropdown-menu">
											<li[#if isFine?? && isFine] class="active"[/#if] data-filter-property="isFine" data-filter-value="true">
												<a href="javascript:;">${message("admin.product.isFine")}</a>
											</li>
											<li[#if isFine?? && !isFine] class="active"[/#if] data-filter-property="isFine" data-filter-value="false">
												<a href="javascript:;">${message("admin.product.notFine")}</a>
											</li>
											<li class="divider"></li>
											<li[#if isHot?? && isHot] class="active"[/#if] data-filter-property="isHot" data-filter-value="true">
												<a href="javascript:;">${message("admin.product.isHot")}</a>
											</li>
											<li[#if isHot?? && !isHot] class="active"[/#if] data-filter-property="isHot" data-filter-value="false">
												<a href="javascript:;">${message("admin.product.notHot")}</a>
											</li>
											<li class="divider"></li>
