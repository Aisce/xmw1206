	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var $cancelHot = $("#cancelHot");
				var $ids = $("input[name='ids']");
				
				// 删除
				$delete.on("success.shopxx.delete", function() {
					$cancelHot.attr("disabled", true);
				});
				
				// 取消热门产品
				$cancelHot.click(function() {
					bootbox.confirm("${message("admin.product.cancelHotConfirm")}", function(result) {
						if (result == null || !result) {
							return;
						}
						
						$.post("${base}/admin/product_hot/cancel_hot", $("input[name='ids']").serialize(), function() {
				...
				// ID多选框
				$ids.change(function() {
					$cancelHot.attr("disabled", $("input[name='ids']:checked").length < 1);
				});
...
<body class="admin">
	...
	<main>
		<div class="container-fluid">
			...
			<form action="${base}/admin/product_hot/list" method="get">
						...
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									...
									<button id="cancelHot" class="btn btn-default" type="button" data-toggle="shelf" disabled>
										<i class="iconfont icon-moreunfold"></i>
										${message("admin.product.cancelHot")}
									</button>
									<div class="btn-group">
