...
<head>
	...
	<title>${message("admin.trainv.list")} - 北京芯梦国际科技有限公司</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.trainv.list")}</li>
			</ol>
			<form action="${base}/admin/trainv/list" method="get">
					...
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									<a class="btn btn-default" href="${base}/admin/trainv/add" data-redirect-url="${base}/admin/trainv/list">
									...
									<div class="input-group-btn">
										<button class="btn btn-default" type="button" data-toggle="dropdown">
											[#switch page.searchProperty]
												[#default]
													<span>${message("Trainv.subject")}</span>
											[/#switch]
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li[#if !page.searchProperty?? || page.searchProperty == "subject"] class="active"[/#if] data-search-property="subject">
												<a href="javascript:;">${message("Trainv.subject")}</a>
											</li>
										</ul>
									</div>
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="subject">
												${message("Trainv.subject")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;">
												${message("Trainv.sponsor")}
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="isPublication">
												${message("Trainv.isPublication")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
									...
									<tbody>
										[#list page.content as trainv]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${trainv.id}">
														<label></label>
													</div>
												</td>
												<td>${trainv.subject}</td>
												<td>${trainv.sponsor}</td>
												<td>
													[#if trainv.isPublication]
												...
												<td>
													<span title="${trainv.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${trainv.createdDate}</span>
												</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/trainv/edit?id=${trainv.id}" title="${message("common.edit")}" data-toggle="tooltip" data-redirect-url>
														<i class="iconfont icon-write"></i>
													</a>
													[#if trainv.isPublication]
														<a class="btn btn-default btn-xs btn-icon" href="${base}/new_trainv/detail?trainvId=${trainv.id}" title="${message("common.view")}" data-toggle="tooltip" target="_blank">
															<i class="iconfont icon-search"></i>
														</a>
													[#else]
														<button class="disabled btn btn-default btn-xs btn-icon" type="button" title="${message("admin.trainv.notPublished")}" data-toggle="tooltip">
															<i class="iconfont icon-search"></i>
														</button>
													[/#if]
												</td>
