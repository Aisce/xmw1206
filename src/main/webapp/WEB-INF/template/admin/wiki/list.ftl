...
<head>
	...
	<title>${message("admin.wiki.list")} - 北京芯梦国际科技有限公司</title>
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("admin.wiki.list")}</li>
			</ol>
			<form action="${base}/admin/wiki/list" method="get">
					...
					<div class="panel-heading">
						<div class="row">
							<div class="col-xs-12 col-sm-9">
								<div class="btn-group">
									<a class="btn btn-default" href="${base}/admin/wiki/add" data-redirect-url="${base}/admin/wiki/list">
							...
							<table class="table table-hover">
								<thead>
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="wikiCategory">
												${message("Wiki.wikiCategory")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="isPublication">
												${message("Wiki.isPublication")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
									...
									<tbody>
										[#list page.content as wiki]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${wiki.id}">
														<label></label>
													</div>
												</td>
												<td>${wiki.title}</td>
												<td>${wiki.wikiCategory.name}</td>
												<td>
													[#if wiki.isPublication]
												...
												<td>
													<span title="${wiki.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${wiki.createdDate}</span>
												</td>
												<td>
													<a class="btn btn-default btn-xs btn-icon" href="${base}/admin/wiki/edit?id=${wiki.id}" title="${message("common.edit")}" data-toggle="tooltip" data-redirect-url>
														<i class="iconfont icon-write"></i>
													</a>
													[#if wiki.isPublication]
														<a class="btn btn-default btn-xs btn-icon" href="${base}${wiki.path}" title="${message("common.view")}" data-toggle="tooltip" target="_blank">
															<i class="iconfont icon-search"></i>
														</a>
													[#else]
														<button class="disabled btn btn-default btn-xs btn-icon" type="button" title="${message("admin.wiki.notPublished")}" data-toggle="tooltip">
															<i class="iconfont icon-search"></i>
														</button>
													[/#if]
												</td>
