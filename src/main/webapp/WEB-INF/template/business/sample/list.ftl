	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("business.sample.list")}</li>
			</ol>
			<form action="${base}/admin/member_real/list" method="get">
								...
								<div id="search" class="input-group">
									<div class="input-group-btn">
										<button class="btn btn-default" type="button" data-toggle="dropdown">
											<span>${message("business.sample.applyUser")}</span>
											<span class="caret"></span>
										</button>
										<ul class="dropdown-menu">
											<li data-search-property="applyUser">
												<a href="javascript:;">${message("business.sample.applyUser")}</a>
											</li>
										</ul>
									</div>
									...
									<tr>
										...
										<th>
											<a href="javascript:;" data-order-property="product">
												${message("business.sample.product")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
											<a href="javascript:;" data-order-property="applyUser">
												${message("business.sample.applyUser")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
										<th>
										...
										<th>
											<a href="javascript:;" data-order-property="flag">
												${message("business.sample.flag")}
												<i class="iconfont icon-biaotou-kepaixu"></i>
											</a>
										</th>
									...
									<tbody>
										[#list page.content as sample]
											<tr>
												<td>
													<div class="checkbox">
														<input name="ids" type="checkbox" value="${sample.id}">
														<label></label>
													</div>
												</td>
												<td>${sample.product.name}</td>
												<td>${sample.applyUser}</td>
												<td>
													<span title="${sample.createdDate?string("yyyy-MM-dd HH:mm:ss")}" data-toggle="tooltip">${sample.createdDate}</span>
												</td>
												<td>
													[#if sample.flag== 0]
														<span class="text-red">${message("business.sample.flag.zhong")}</span>
													[#elseif sample.flag == 1]
														<span class="text-green">${message("business.sample.flag.tong")}</span>
													[#else]
														<span class="text-black">${message("business.sample.flag.shi")}</span>
													[/#if]
												</td>
												<td>
													[#if sample.flag== 0]
														<a class="btn btn-default btn-xs btn-icon" href="${base}/business/sample/edit?id=${sample.id}" title="${message("business.sample.shenhe")}" data-toggle="tooltip" data-redirect-url>
															<i class="iconfont icon-write"></i>
														</a>
													[#elseif sample.flag == 1]
														<a class="btn btn-default btn-xs btn-icon" href="${base}/business/sample/edit?id=${sample.id}" title="查看" data-toggle="tooltip" data-redirect-url>
															<i class="iconfont icon-write"></i>
														</a>
													[#else]
														<a class="btn btn-default btn-xs btn-icon" href="${base}/business/sample/edit?id=${sample.id}" title="查看" data-toggle="tooltip" data-redirect-url>
