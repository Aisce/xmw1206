...
[#noautoesc]
	[#escape x as x?js_string]
		...
		+function($) {
			...
			Checkout.prototype.execute = function(data) {
				var that = this;
				var skuId = typeof this.options.skuId === "function" ? this.options.skuId(this.$element) : this.options.skuId;
				var quantity = typeof this.options.quantity === "function" ? this.options.quantity(this.$element) : this.options.quantity;
			...
			$(document).on("click.shopxx.checkout.data-api", "[data-action='checkout']", function() {
				...
				if($(this).hasClass('dlcg')){
					Plugin.call($this, ["execute", {value:1}]);
				}else{
					Plugin.call($this, ["execute", {value:0}]);
				}
			});
		...
		(function($) {
			...
			// 结算
			$.checkout = function(skuId, quantity, data) {
				if (skuId != null) {
					$.get(checkSkuUrl, {
						skuId: skuId,
						quantity: quantity
					...
					$.get(checkCartUrl).done(function() {
						location.href = checkoutUrl + (data?'?type='+data.value:'');
					});
				}
			};
		...
		$().ready(function() {
			...
			$redirectUrl.click(function() {
				...
				location.href = URI(href).setSearch(redirectUrlParameterName, encodeURIComponent(redirectUrl)).toString();
				return false;
			});
