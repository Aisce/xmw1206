...
<head>
	...
	[#if productCategory??]
		[@seo type = "PRODUCT_LIST"]
			...
			[/#if]
			<title>[#if productCategory.seoTitle?has_content]${productCategory.seoTitle}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - 北京芯梦国际科技有限公司[/#if]</title>
		[/@seo]
	[#else]
		...
		<%_.each(data, function(product, i) {%>
			<li class="list-item">
				<a href="${base}<%-product.path%>">
					<img style="width : 166px;height : 164px" class="img-responsive center-block" src="<%-product.thumbnail != null ? product.thumbnail : "${setting.defaultThumbnailProductImage}"%>" alt="<%-product.name%>">
				</a>
				<a href="${base}<%-product.path%>">
	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var url = "";
				var storeProductCategory = $("#storeProductCategoryId").val();
				
				if(storeProductCategory != null && storeProductCategory != ""){
					url = "${base}/product/list?storeProductCategoryId=-1&storeId=${storeProductCategory.store.id}&pageNumber=<%-pageNumber%>";
				}else{
...
<input type="hidden" id="storeProductCategoryId" value="${storeProductCategory.id}"/>
	[#if productCategory??]
