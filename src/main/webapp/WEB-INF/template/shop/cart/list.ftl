...
<head>
	...
	<title>${message("shop.cart.title")}[#if showPowered] - 北京芯梦国际科技有限公司[/#if]</title>
			...
			[#if currentCart?? && !currentCart.isEmpty()]
				<table>
					<thead>
						<tr>
							<th>编号</th>
							<th width="400">${message("shop.cart.sku")}</th>
							<th>型号</th>
							<th>封装形式</th>
							<th>质量等级</th>
							<th>厂家</th>
						...
						<tbody class="cart-item-group">
							...
							[#list cartItems as cartItem]
								<tr>
									...
									<td>${cartItem.sku.product.cpxh}</td>
									<td>${cartItem.sku.product.fzxs}</td>
									<td>${cartItem.sku.product.zldj}</td>
									<td>
									<a href="${base}${store.path}" target="_blank">${store.name}</a>
									[#if store.type == "SELF"]
										<span class="label label-primary">${message("Store.Type.SELF")}</span>
				...
				<div class="bar">
					<div class="row">
						...
						<div class="col-xs-1">
						<button class="btn btn-primary btn-lg btn-block dlcg" type="button" data-action="checkout">辅助采购</button>
						</div>
					</div>
				</div>
			[#else]
