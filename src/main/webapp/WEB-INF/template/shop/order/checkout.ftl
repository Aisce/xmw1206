...
<head>
	...
	<title>${message("shop.order.checkout")}[#if showPowered] - 北京芯梦国际科技有限公司[/#if]</title>
	...
	<main>
		...
		[/#if]
			<form id="orderForm" class="ajax-form form-horizontal" action="${base}/order/create" method="post">
				...
				[/#if]
				<input id="paymentMethodId" name="paymentMethodId" type="hidden" value="2">
				...
				<table>
					<thead>
						<tr>
							<th>编号</th>
							<th width="400">${message("shop.order.sku")}</th>
							<th>型号</th>
							<th>封装形式</th>
							<th>质量等级</th>
							<th>厂家</th>
					...
					[#list orders as order]
						<tbody class="order-item-group">
							...
							[#list order.orderItems as orderItem]
								<tr>
									<td>${orderItem.sku.product.sn}</td>
									<td>
										<div class="media">
									...
									<td>${orderItem.sku.product.cpxh}</td>
									<td>${orderItem.sku.product.fzxs}</td>
									<td>${orderItem.sku.product.zldj}</td>
									<td>
										<a href="${base}${order.store.path}" target="_blank">${order.store.name}</a>
										[#if order.store.type == "SELF"]
											<span class="label label-primary">${message("Store.Type.SELF")}</span>
				...
				<div class="bar">
					<div class="row">
						...
						<div class="col-xs-1">
							<button class="btn btn-primary btn-lg btn-block" type="submit" style="width:125px">提交辅助采购</button>
						</div>
					</div>
				</div>
			</form>
