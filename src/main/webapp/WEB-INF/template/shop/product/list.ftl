...
<head>
		...
		[@seo type = "PRODUCT_LIST"]
			...
			<title>
				...
				[#else]
					${seo.resolveTitle()}
				[/#if]
				[#if showPowered] - 北京芯梦国际科技有限公司[/#if]
			</title>
		[/@seo]
	[#else]
		<title>${message("shop.product.list")}[#if showPowered] - 北京芯梦国际科技有限公司[/#if]</title>
	[/#if]
		...
		<li>
			...
			<div class="media">
				...
				<div class="media-body media-middle">
					...
					<a class="delete-compare btn btn-default btn-xs btn-icon" href="javascript:;" title="${message("common.delete")}" id="<%-compareProduct.id%>" data-product-id="<%-compareProduct.id%>">
	...
	<style type="text/css">
		...
		.checkboxi{
			left: 5px;
			top: 5px;
			background: url(${base}/resources/shop/images/search.ele.png) no-repeat 9999px 9999px;
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				if (compareProductIds.length > 0) {
					...
					//获取商品id
					//$(".add-compare").data("product-id")
					for(var i=0;i<compareProductIds.length;i++){
						if(null!=$("#"+compareProductIds[i])[0]){
							$("#"+compareProductIds[i])[0].lastElementChild.style.display="inline-block";
							$("#"+compareProductIds[i])[0].firstElementChild.style.display="none";
						}
					}
				...
				$addCompare.click(function() {
					...
					//获取未选择的class的名称
					$element[0].firstElementChild.style.display="none";
					//获取选择的class 名称
					$element[0].lastElementChild.style.display="inline-block";
					
					
					//判断产品在对比栏中是否存在
					if ($.inArray(productId, compareProductIds) >= 0) {
						var $elementdelete = $("#"+productId);
						var deleteProductId = $elementdelete.data("product-id");
						
						//获取未选择的class的名称
						$element[0].firstElementChild.style.display="inline-block";
						//获取选择的class 名称
						$element[0].lastElementChild.style.display="none";
						
						$elementdelete.closest("li").velocity("fadeOut").remove();
				...
				$compareBar.on("click", "a.delete-compare", function() {
					...
					$(".list .list-item .add-compare[data-product-id="+productId+"]").children(".checkboxiSelect").css('display','none').siblings().css('display','inline-block')
				...
				$clearCompareBar.click(function() {
					for(var i=0;i<compareProductIds.length;i++){
						console.log($(".list .list-item #"+compareProductIds[i]));
						
						//$(".clearfix #"+compareProductIds[i])[0].lastElementChild.style.display="none";
						$(".list .list-item .add-compare[data-product-id="+compareProductIds[i]+"]").children(".checkboxiSelect").css('display','none').siblings().css('display','inline-block')
						
						//console.log($(".clearfix .list-item #"+compareProductIds[i]));
						//$("#"+compareProductIds[i])[0].firstElementChild.style.display="inline-block"
					}
					compareProductIds = [];
					localStorage.removeItem(compareProductIdsLocalStorageKey);
		...
		<!--对比栏-->
			...
			<div class="row">
				...
				<div class="col-xs-10">
					...
					[#if storeProductCategory.id == '-1' && storeProductCategory.store.id??]
						<form id="productForm" action="${base}/product/list?storeProductCategoryId=-1&storeId=${storeProductCategory.store.id}" method="get">
							...
							<div class="list">
								<ul class="clearfix">
									[#list page.content as product]
										...
										<li class="list-item" style="height:270px">
											<div class="list-item-body">
												<a href="${base}/product/detail/${product.id}" target="_blank">
													<div style="width:187px;height:125px; background:url(${product.thumbnail!setting.defaultThumbnailProductImage});background-repeat:no-repeat;background-size:contain;background-position: center;"> </div>
													[#if false]
														<img id="productImage${product.id}" class="lazy-load img-responsive center-block" src="${base}/resources/shop/images/default_large_product_image.png" alt="${product.name}" data-original="${product.thumbnail!setting.defaultThumbnailProductImage}" style="z-index:1;transform:scale(0.8);width: 188px;height: 124px;">
													[/#if]
													
												</a>
												<a href="${base}${product.path}" target="_blank">
													<h5 class="text-overflow" title="${product.name}" style="margin-top: 25px;">${product.name}</h5>
													[#if product.caption?has_content]
														<h6 class="text-overflow" title="${product.caption}">${product.caption}</h6>
													[/#if]
												</a>
												<p class="text-center">
													<a href="${base}/product/list?storeProductCategoryId=-1&storeId=${product.store.id}" title="${product.store.name}" target="_blank">${abbreviate(product.store.name, 15)}</a>
											...
											<div class="list-item-footer clearfix">
												[#if product.type == "GENERAL"]
													...
													[#else]
														...
														[#else]
															<a href="${base}/member/realname/edit">
																<i class="iconfont icon-cart"></i>
																${message("shop.product.addCart")}
															</a>
														[/#if]
												...
												[/#if]
												[#if currentUser == null]
													<a class="add-product-favorite" href="${base}/member/login" title="${message("shop.product.addProductFavorite")}">
														<i class="iconfont icon-like"></i>
													</a>
													<a class="add-compare" href="${base}/member/login" title="${message("shop.product.addCompare")}" id="${product.id}" data-product-id="${product.id}">
														对比
													</a>
												[#elseif currentUser.attestationFlag == 'REAL']
													...
													<a class="add-compare" href="javascript:;" title="${message("shop.product.addCompare")}" id="${product.id}" data-product-id="${product.id}">
														对比
														<i class="checkboxi"></i>
														<i class="checkboxi checkboxiSelect"></i>
													</a>
												[#else]
													<a class="add-product-favorite" href="${base}/member/realname/edit" title="${message("shop.product.addProductFavorite")}">
														<i class="iconfont icon-like"></i>
													</a>
													<a class="add-compare" href="${base}/member/realname/edit" title="${message("shop.product.addCompare")}" id="${product.id}" data-product-id="${product.id}">
														对比
													</a>
												[/#if]
											</div>
