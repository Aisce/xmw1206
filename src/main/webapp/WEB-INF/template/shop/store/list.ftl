...
<head>
	...
	<link href="${base}/resources/shop/store/list.less" type="text/css" rel="stylesheet/less">
	<!--[if lt IE 9]>
						...
						<div class="search">
							<ul class="J_valueList" style="width:100%;display:flex;justify-content:space-between;">
								[#list findAll as find]
									<li style="width: 130px;margin-right: 5px; float: left;text-align:center;">
								...
								<li class="list-item">
									<div class="list-item-body">
										<a href="${base}/product/list?storeProductCategoryId=-1&storeId=${store.id}" target="_blank">
											<img id="productImage1151" class="lazy-load img-responsive center-block" src="${store.logo!setting.defaultThumbnailProductImage}" style="transform:scale(0.8);height:121px">
										</a>
										<p class="text-center">
											<a href="${base}/product/list?storeProductCategoryId=-1&storeId=${store.id}" title="${store.name}" target="_blank">${store.name}</a>
										</p>
									</div>
								</li>
							[/#list]
